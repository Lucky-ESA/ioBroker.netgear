...
class NeatGear extends EventEmitter {
    ...
    async login(first) {
        ...
        if (resp && resp.headers && resp.headers["set-cookie"]) {
            ...
            this.adapter.log.debug(`login data: ${resp.data}`);
                    ...
                    if (
                        ...
                        if (new_uri != null && new_uri[1] != null) {
                            ...
                            if (
                                ...
                                for (const ca of cables) {
                                    const port = load_info.data.match(
                                        "TesterEntry" + (ca - 1).toString() + "='(.*?)';",
                                    );
    ...
    async changeSpeed() {
        ...
        ports && ports.data && this.adapter.log.debug(`loadPortStatus: ${ports.data}`);
        if (
            ports != null &&
            ports.data &&
