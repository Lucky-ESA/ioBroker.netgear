...
class NeatGear extends EventEmitter {
    ...
    async changeName(name) {
        //Post /config/status_switch_info.htm
        const uri = `/config/status_switch_info.${this.config.protocol}`;
        const info = await this.sendRequest("GET", uri, null);
        ...
        if (info && info.data) {
            ...
            if (data != null && data[1] != null && rand != null && rand[1] != null) {
                const infos = data[1].split("?");
                if (infos.length > 0) {
                    //'JGS524PE?Netgear-Keller?08-36-c9-18-16-89?2.6.0.22?Enable?192.168.2.59?255.255.255.0?192.168.2.1?3KL9935N0158A';
                    ...
                    const change = await this.sendRequest("POST", uri, data);
                    this.adapter.log.info("VAL " + JSON.stringify(val));
                    this.adapter.log.info("CHANGE " + change.data);
                    try {
                        this.adapter.log.info(JSON.stringify(change));
                    } catch (e) {
                        this.adapter.log.info(JSON.stringify(change.headres));
                        this.adapter.log.info(JSON.stringify(e));
                    }
                    const infoss = await this.sendRequest("GET", uri, null);
                    this.adapter.log.info(infoss.data);
                } else {
                    this.adapter.log.warn(`changeName no value: ${data[1]}`);
                    this.adapter.log.warn(`changeName no secure: ${rand}`);
                }
            }
        } else {
            this.adapter.log.warn(`changeName no data: ${info}`);
        }
    }
