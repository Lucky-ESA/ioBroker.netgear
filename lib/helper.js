module.exports = {
    ...
    async createInfos(id, first) {
        ...
        await this.createDataPoint(`${id.dp}.Ports.Port_${`0${first[0]}`.slice(-2)}.status`, common, "state", first[1]);
        common = {
            name: {
                en: "Speed",
                de: "Geschwindigkeit",
        ...
        await this.createDataPoint(`${id.dp}.Ports.Port_${`0${first[0]}`.slice(-2)}.speed`, common, "state", first[2]);
        common = {
            name: {
                en: "Connection speed",
    ...
    async createDataPoint(ident, common, types, value, native) {
        ...
        if (value != null) {
            await this.setState(ident, { val: value, ack: true });
        }
    },
};
