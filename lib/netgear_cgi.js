...
class NeatGear extends EventEmitter {
    ...
    async loadPortStatus(first, value_status) {
        ...
        if (port != null && port.data && typeof port.data === "string" && port.data.indexOf("portStatus") != -1) {
            ...
            if (td.length > 0) {
                ...
                if (value_status) {
                    await this.adapter.setState(`${this.config.dp}.Ports.json`, {
            ...
            if (td.length > 0) {
                for (let i = 0; i < td.length; i++) {
                    ...
                    if (rows.length === 3) {
                        ...
                        if (this.json_array && this.json_array[i]) {
    ...
    async loadBitRate(first, value_status) {
        ...
        rate && this.adapter.log.debug(`rate: ${rate.data}`);
                    ...
                    if (cables != null && Array.isArray(cables) && cables.length > 0) {
                        ...
                        if (change && change.data) {
                            ...
                            if (td.length > 0) {
                                ...
                                await this.adapter.setState(`${this.config.dp}.Remote.cable_testing_result`, {
                                    val: JSON.stringify(arr_json),
                                    ack: true,
                                });
                            } else {
    ...
    async setStatesInfo(infos) {
        ...
        await this.adapter.setState(`${dp}.bootloader`, { val: infos[9], ack: true });
